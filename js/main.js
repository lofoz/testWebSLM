...
ADSR.prototype.processSample = function(sample) {
	...
	if (this.samplesProcessed <= this.attack) {
		...
		amplitude = this.sustainLevel + (0 - this.sustainLevel) * ((this.samplesProcessed - this.sustain) / (this.release -
			this.sustain));
	}
...
function Biquad(type, sampleRate) {
	...
	this.process = function(buffer) {
		...
		for (var i = 0; i < buffer.length; i++) {
			output[i] = this.b0a0 * buffer[i] + this.b1a0 * this.x_1_l + this.b2a0 * this.x_2_l - this.a1a0 * this.y_1_l - this
				.a2a0 * this.y_2_l;
	...
	this.processStereo = function(buffer) {
		...
		for (var i = 0; i < len / 2; i++) {
			output[2 * i] = this.b0a0 * buffer[2 * i] + this.b1a0 * this.x_1_l + this.b2a0 * this.x_2_l - this.a1a0 * this.y_1_l -
			...
			output[2 * i + 1] = this.b0a0 * buffer[2 * i + 1] + this.b1a0 * this.x_1_r + this.b2a0 * this.x_2_r - this.a1a0 *
				this.y_1_r - this.a2a0 * this.y_2_r;
...
DSP.freqz = function(b, a, w) {
	...
	for (i = 0; i < w.length; i++) {
		...
		result[i] = sqrt(numerator.real * numerator.real + numerator.imag * numerator.imag) / sqrt(denominator.real *
			denominator.real + denominator.imag * denominator.imag);
	}
...
function GraphicalEq(sampleRate) {
	...
	this.recalculateFreqz = function(bandIndex) {
		...
		if (bandIndex < 0 || bandIndex > (this.filters.length - 1)) {
			throw "The band index of the graphical equalizer is out of bounds. " + bandIndex + " is out of [" + 0 + ", " + this
				.filters.length - 1 + "]";
		}
...
function Reverb(maxDelayInSamplesSize, delayInSamples, masterVolume, mixVolume, delayVolume, dampFrequency) {
	...
	for (i = 0; i < this.NR_OF_MULTIDELAYS; i++) {
		...
		this.multiDelays[i] = new MultiDelay(maxDelayInSamplesSize, Math.round(this.delayInSamples * delayMultiply), this.masterVolume,
			this.delayVolume);
	}
}
...
Reverb.prototype.process = function(interleavedSamples) {
	...
	for (i = 0; i < this.NR_OF_SINGLEDELAYS; i++) {
		...
		singleDelaySamples = DSP.mixSampleBuffers(singleDelaySamples, this.singleDelays[i].process(outputSamples), 2 % i ===
			0, 1);
	}
