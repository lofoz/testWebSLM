	...
	.then(function(mediaStream) {
		...
		node.onaudioprocess = function(data) {
			...
			for (var i = 0; i < out.length / 2; i += 2) {
				...
				// values += array[i]/255;
				// values += Math.pow(out[i]/2048, 2);
				var value = Math.sqrt(out[i] * out[i] + out[i + 1] * out[i + 1]);
				values += Math.pow(value, 2);
			...
			// }
			// values /= (2048 * 16);
			console.log('FFT  ' + String(10 * Math.log10(values / 4096 * 2500000000.0)));
		...
		// var cnt = parseInt(returnData["dBA_ref_acc"].length / 15);
